...
dat %>% group_by(ID) %>%
  ...
  select(wind.spd, wind.spd.sum, wind.dir, wind.dir.sum,
         cos, sin, days, type, dist, year, ID, ATOS, month.n) -> dat3_df
...
lme.out <- list(dist = fit_lme,
                ATPS = fit_lme_ATOS,
                data = dat3_df)
saveRDS(lme.out, file = "RData/results_lme_2019-03-14.rds")
